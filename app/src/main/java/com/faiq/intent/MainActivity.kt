...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(p0: View?) {
        when (p0?.id) {
            R.id.btn_move -> {
                val moveIntent = Intent(this@MainActivity, EmptyActivity::class.java)
                startActivity(moveIntent)
            }

            R.id.btn_move_activity_data -> {
                val moveWithDataIntent = Intent(this@MainActivity, MoveWithData::class.java)
                moveWithDataIntent.putExtra(MoveWithData.EXTRA_NAME, "Dicoding Academy Bro")
                moveWithDataIntent.putExtra(MoveWithData.EXTRA_AGE, 10)
                startActivity(moveWithDataIntent)
            }
            R.id.btn_dial_number -> {
                val phoneNumber = "081298419634"
